...
impl AfpacketSender {
    ...
    fn check_arp(&mut self, now: u64, arp: &Arc<NpbArpTable>) -> IOResult<u32> {
        ...
        if entry.is_none() {
            return Err(IOError::new(ErrorKind::Other, format!("Arp not found: {}", self.remote)));
        }
        ...
        if if_name != self.if_name {
            ...
            if af_packet.is_err() {
                return Err(IOError::new(ErrorKind::Other, format!("Afpacket init failed with: {:?}", options)))
            }
...
impl NpbArpTable {
    ...
    pub fn add(&self, remote: &IpAddr) {
        ...
        self.table.write().unwrap().insert(remote.clone(), ArpEntry{
            dst_ip: remote.clone(),
    ...
    pub fn lookup(&self, remote: &IpAddr) -> Option<(MacAddr, MacAddr, IpAddr, String)> {
        if let Some(entry) = self.table.read().unwrap().get(&remote) {
            ...
            return Some((entry.src_mac, entry.dst_mac, entry.src_ip, entry.if_name.clone()))
        }
        self.add(remote);
        return None;
    }
...
impl NpbConnectionPool {
    ...
    fn send_to(&mut self, timestamp: u64, underlay_l2_opt_size: usize, packet: Vec<u8>) -> IOResult<usize> {
        ...
        if conn.is_some() {
            return conn
                .as_mut()
                .unwrap()
                .send(timestamp, underlay_l2_opt_size, header_size, packet, &self.arp);
        }
        ...
        let ret = conn.send(timestamp, underlay_l2_opt_size, header_size, packet, &self.arp);
        self.connections.insert(key, conn);
        return ret;
    }

    pub fn send(&mut self, timestamp: u64, underlay_l2_opt_size: usize, packet: Vec<u8>) -> IOResult<usize> {
